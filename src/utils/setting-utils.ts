...
const UI_PREF_COOKIE_MAX_AGE = 60 * 60 * 24 * 365;

function readPreferenceCookie(key: string): string | null {
...
function writePreferenceCookie(key: string, value: string): void {
	if (typeof document === "undefined") {
		return;
	}

	document.cookie = `${encodeURIComponent(key)}=${encodeURIComponent(value)}; Max-Age=${UI_PREF_COOKIE_MAX_AGE}; Path=/; SameSite=Lax`;
}

function readPreferenceValue(key: string): string | null {
	...
	const cookieValue = readPreferenceCookie(key);
	if (cookieValue) {
...
export function getHue(): number {
	const stored = readPreferenceValue("hue");
...
export function setHue(hue: number): void {
	...
	writePreferenceCookie("hue", value);
...
export function setTheme(theme: LIGHT_DARK_MODE): void {
	...
	writePreferenceCookie("theme", theme);
	applyThemeToDocument(theme);
}

export function getStoredTheme(): LIGHT_DARK_MODE {
	return (readPreferenceValue("theme") as LIGHT_DARK_MODE) || DEFAULT_THEME;
}

export function getStoredWallpaperMode(): WALLPAPER_MODE {
	return (
		(readPreferenceValue("wallpaperMode") as WALLPAPER_MODE) ||
...
export function setWallpaperMode(mode: WALLPAPER_MODE): void {
	...
	writePreferenceCookie("wallpaperMode", mode);

...
import { musicPlayerConfig as buildTimeMusicPlayerConfig } from "@/config";
...
function getRuntimeMusicPlayerConfig(): MusicPlayerConfig {
	if (typeof window === "undefined") {
		return buildTimeMusicPlayerConfig;
	}

	return window.siteConfig?.musicPlayer ?? buildTimeMusicPlayerConfig;
}
...
class MusicPlayerStore {
	...
	private unregisterSiteConfigListener: (() => void) | undefined;
	...
	async initialize(): Promise<void> {
		...
		this.registerSiteConfigListener();

		if (!getRuntimeMusicPlayerConfig().enable) {
			return;
		}
	...
	private registerSiteConfigListener(): void {
		if (typeof document === "undefined" || this.unregisterSiteConfigListener) {
			return;
		}

		const handler = () => {
			void this.applyRuntimeConfig();
		};

		document.addEventListener("site-config-updated", handler);
		this.unregisterSiteConfigListener = () => {
			document.removeEventListener("site-config-updated", handler);
		};
	}

	private async applyRuntimeConfig(): Promise<void> {
		const runtimeConfig = getRuntimeMusicPlayerConfig();
	...
	private async loadPlaylist(): Promise<void> {
		const runtimeConfig = getRuntimeMusicPlayerConfig();
		const mode = runtimeConfig.mode ?? "meting";
		const meting_api =
			runtimeConfig.meting_api ??
			"https://www.bilibili.uno/api?server=:server&type=:type&id=:id&auth=:auth&r=:r";
		const meting_id = runtimeConfig.id ?? "14164869977";
		const meting_server = runtimeConfig.server ?? "netease";
		const meting_type = runtimeConfig.type ?? "playlist";
	...
	destroy(): void {
		...
		if (this.unregisterSiteConfigListener) {
			this.unregisterSiteConfigListener();
			this.unregisterSiteConfigListener = undefined;
		}
		if (this.audio) {
			this.audio.pause();
			this.audio.src = "";

<script lang="ts">
	...
	let runtimeConfig: MusicPlayerConfig = buildTimeMusicPlayerConfig;
	let showFloatingPlayer = runtimeConfig.showFloatingPlayer;
	let floatingEntryMode = runtimeConfig.floatingEntryMode ?? "default";
	let useFabEntry = floatingEntryMode === "fab";
	let shouldRenderFloatingUi =
		showFloatingPlayer && runtimeConfig.enable;
	let unsubscribe: (() => void) | undefined;
	let configUpdateHandler: (() => void) | undefined;

	$: showFloatingPlayer = runtimeConfig.showFloatingPlayer;
	$: floatingEntryMode = runtimeConfig.floatingEntryMode ?? "default";
	$: useFabEntry = floatingEntryMode === "fab";
	$: shouldRenderFloatingUi = showFloatingPlayer && runtimeConfig.enable;
	...
	onMount(() => {
		runtimeConfig = getRuntimeMusicPlayerConfig();
		configUpdateHandler = () => {
			runtimeConfig = getRuntimeMusicPlayerConfig();
		};
		document.addEventListener("site-config-updated", configUpdateHandler);

<script lang="ts">
	...
	import type { PasswordModalProps } from "./types/auth";
	import {
	...
	} from "./utils/unlock-view";

	const { hint = "", postId }: PasswordModalProps = $props();
	...
	function getLoadingText(): string {
		return i18n(I18nKey.passwordVerifying);
	}
	...
	function resolveErrorMessage(error: unknown): string {
		...
		return i18n(I18nKey.passwordDecryptRetry);
	}

	async function attemptUnlock(inputPassword: string): Promise<void> {
		const normalizedPassword = inputPassword.trim();
		if (!normalizedPassword) {
			errorMessage = i18n(I18nKey.passwordRequired);
			return;
		}
		...
		try {
			...
			if (!contentDiv) {
				throw new Error("decrypted-content container not found");
			}

			if (typeof postId !== "number") {
				contentDiv.innerHTML = contentDiv.dataset.content || "";
			}

			await executeDecryptedScripts(contentDiv);
			...
			savePassword(normalizedPassword);
		} catch (error) {
			removeSavedPassword();
	...
	onMount(() => {
		...
		if (savedPassword) {
			...
			void attemptUnlock(savedPassword);
		}
	});
</script>
	...
	<div class="password-container card-base">
		...
		<form class="password-form" onsubmit={handleSubmit}>
			...
			<button
				...
				disabled={isLoading}
			>
				{isLoading ? getLoadingText() : i18n(I18nKey.passwordUnlock)}
			</button>
		</form>

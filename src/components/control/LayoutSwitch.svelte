<script lang="ts">
	...
	function readCookieLayout(): LayoutMode | null {
		if (typeof document === "undefined") {
			return null;
		}

		const encodedKey = `${encodeURIComponent("postListLayout")}=`;
		const rawCookies = document.cookie ? document.cookie.split("; ") : [];

		for (const item of rawCookies) {
			if (item.startsWith(encodedKey)) {
				const value = decodeURIComponent(item.slice(encodedKey.length));
	...
	function setCookieLayout(layout: LayoutMode) {
		if (typeof document === "undefined") {
			return;
		}

		document.cookie = `${encodeURIComponent("postListLayout")}=${encodeURIComponent(layout)}; Max-Age=${ONE_YEAR_SECONDS}; Path=/; SameSite=Lax`;
	}
	...
	function updateStorage(layout: LayoutMode) {
		...
		setCookieLayout(layout);
	}

	function getSavedLayout(): LayoutMode | null {
		const sessionLayout = sessionStorage.getItem("postListLayout");
		if (sessionLayout === "list" || sessionLayout === "grid") {
			return sessionLayout;
		}

		const localLayout = localStorage.getItem("postListLayout");
		if (localLayout === "list" || localLayout === "grid") {
			return localLayout;
		}

		return readCookieLayout();
	}

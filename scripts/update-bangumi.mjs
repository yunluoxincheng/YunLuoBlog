...
async function getUserIdFromConfig() {
	try {
		...
		const userIdFromData = configData?.site?.bangumi?.userId;
		if (typeof userIdFromData === "string" && userIdFromData.trim()) {
			if (
				userIdFromData === "your-bangumi-id" ||
				userIdFromData === "your-user-id"
			) {
				console.warn(
					"Warning: bangumi.userId appears to be a default value.",
				);
			}
		...
		throw new Error("Could not find bangumi.userId in config sources");
	} catch (error) {
		console.error("✘ Failed to read Bangumi ID from config sources");
		throw error;
	}
}

...
async function getUserIdFromConfig() {
	try {
		...
		const vmidFromData = configData?.site?.bilibili?.vmid;
		if (typeof vmidFromData === "string") {
			const normalized = vmidFromData.trim();
			if (!normalized) {
				console.warn("Warning: bilibili.vmid is empty.");
				return null;
			}
			return normalized;
		}
		...
		throw new Error("Could not find bilibili.vmid in config sources");
	} catch (error) {
		console.error("✘ Failed to read Bilibili vmid from config sources");
		throw error;
	}
}
...
async function getCoverMirrorFromConfig() {
	try {
		const configData = await readConfigData();
		const coverMirror = configData?.site?.bilibili?.coverMirror;
		if (typeof coverMirror === "string") {
			return coverMirror;
		}
...
async function getUseWebpFromConfig() {
	try {
		const configData = await readConfigData();
		const useWebp = configData?.site?.bilibili?.useWebp;
		if (typeof useWebp === "boolean") {
			return useWebp;
		}
...
async function main() {
	...
	if (!VMID) {
		console.error(
			"✘ Bilibili vmid is not set. Please set it in site.bilibili.vmid",
		);
		process.exit(1);
	}
